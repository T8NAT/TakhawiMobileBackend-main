// THIS FILE TO APPLY OBSERVER PATTERN
import { EventEmitter } from 'events';
import { NewOfferType } from '../types/eventListnerType';
import { io } from './socket';
import { serializeVehicle } from './serialization/vehicle.serialization';
import { AcceptOfferResponse, VipTrip } from '../types/vipTripType';
import driverService from '../services/driverService';
import { Location, NearestDrivers, User } from '../types/userType';
import { Warning } from '../types/warningType';
import sendPushNotification from './pushNotification';
import prisma from '../../prisma/client';
import { Roles } from '../enum/roles';
import { Notification } from '../types/notificationType';
import {
  DriverCancelationReturn,
  TripStatusChangeReturn,
  UserCancelationReturn,
} from '../types/tripType';
import { TripStatus, TripType } from '../enum/trip';
import { CreateWaslTrip } from '../types/waslType';
import waslService from '../services/waslService';
import authService from '../services/authService';

// Create an instance of EventEmitter
export const customEventEmitter = new EventEmitter(); // WE EXPORT THIS SO WE CAN USE THIS INSTANCE TO EMIT EVENTS

const tripStatusMessages = (status: string) => {
  let arabicBody = '';
  let englishBody = '';
  let arabictitle = '';
  let englishtitle = '';
  switch (status) {
    case TripStatus.ON_WAY:
      arabicBody = 'السائق في الطريق';
      englishBody = 'The driver is on the way';
      arabictitle = 'السائق في الطريق';
      englishtitle = 'Driver On The Way';
      break;
    case TripStatus.ARRIVED:
      arabicBody = 'وصل سائقك! يُرجى التوجه إلى مكان الالتقاء.';
      englishBody =
        'Your driver has arrived! Please head to the pickup location.';
      arabictitle = 'السائق وصل';
      englishtitle = 'Driver Arrived';
      break;
    case TripStatus.INPROGRESS:
      arabicBody = 'بدأت رحلتك! استمتع بالرحلة';
      englishBody = 'Your ride has started! Enjoy your trip';
      arabictitle = 'بدأت رحلتك!';
      englishtitle = 'Your ride has started!';
      break;
    case TripStatus.COMPLETED:
      arabicBody = 'انتهت رحلتك. شكرًا لاستخدامك خدمتنا!';
      englishBody = 'Your ride has ended. Thank you for using our service!';
      arabictitle = 'الرحلة انتهت';
      englishtitle = 'Trip Completed';
      break;
    default:
      break;
  }
  return {
    arabictitle,
    englishtitle,
    arabicBody,
    englishBody,
  };
};

// NOTIFICATION
customEventEmitter.on('notification', (data: Notification) => {
  // 1- SEND NOTIFICATION TO USER ON SOCKET
  io.to(data.User?.uuid!).emit('notification', {
    id: data.id,
    title:
      data.User?.prefered_language === 'ar' ? data.ar_title : data.en_title,
    body: data.User?.prefered_language === 'ar' ? data.ar_body : data.en_body,
    type: data.type,
    is_read: data.is_read,
    createdAt: data.createdAt,
  });
  // 2- SEND PUSH NOTIFICATION TO USER
  if (
    data.User &&
    data.User.User_FCM_Token &&
    data.User.User_FCM_Token.length > 0
  ) {
    sendPushNotification({
      title:
        data.User?.prefered_language === 'ar' ? data.ar_title : data.en_title,
      body: data.User?.prefered_language === 'ar' ? data.ar_body : data.en_body,
      tokens: data.User.User_FCM_Token.map((token) => token.token),
    });
  }
});

// NEW VIP TRIP
customEventEmitter.on('newVipTrip', async (data: VipTrip) => {
  // 1- SEND NOTIFICATION TO DRIVER ON SOCKET
  const location: Location = {
    lat: data.VIP_Trip!.pickup_location_lat,
    lng: data.VIP_Trip!.pickup_location_lng,
  };

  const distanceRanges = [10, 20, 25, 30];
  let drivers: string[] = [];
  const arabicDriversTokens: string[] = [];
  const englishDriversTokens: string[] = [];

  for (const range of distanceRanges) {
    // eslint-disable-next-line no-await-in-loop
    const nearestDrivers = await driverService.getNearestDrivers(
      location,
      data.gender,
      range,
    );
    const trip = {
      id: data.id,
      status: data.status,
      start_date: data.start_date,
      features: data.features,
      passenger_id: data.VIP_Trip!.passnger_id,
      passenger_phone: data.VIP_Trip!.Passnger!.phone,
      passenger_name: data.VIP_Trip!.Passnger!.name,
      passenger_avatar: data.VIP_Trip!.Passnger!.avatar,
      passenger_rate: data.VIP_Trip!.Passnger!.passenger_rate,
      pickup_location_lat: data.VIP_Trip!.pickup_location_lat,
      pickup_location_lng: data.VIP_Trip!.pickup_location_lng,
      pickup_description: data.VIP_Trip!.pickup_description,
      destination_location_lat: data.VIP_Trip!.destination_location_lat,
      destination_location_lng: data.VIP_Trip!.destination_location_lng,
      destination_description: data.VIP_Trip!.destination_description,
    };

    if (nearestDrivers.length > 0) {
      drivers = nearestDrivers.map((driver: NearestDrivers) => {
        if (driver.prefered_language === 'ar') {
          arabicDriversTokens.push(...driver.tokens);
        } else {
          englishDriversTokens.push(...driver.tokens);
        }
        return driver.uuid!;
      });
      io.to(drivers).emit('new vip trip', { ...trip, distance: range });
      break;
    }
  }
  // 2- SEND PUSH NOTIFICATION TO DRIVER
  if (arabicDriversTokens.length > 0) {
    await sendPushNotification({
      title: 'رحلة خاصة جديدة',
      body: 'يوجد رحلة خاصة جديدة بالمنطقة يمكنك ارسال عرضك',
      tokens: arabicDriversTokens,
    });
  }
  if (englishDriversTokens.length > 0) {
    await sendPushNotification({
      title: 'New VIP Trip',
      body: 'You have a new VIP Trip in the area. Hurry up and send your offer',
      tokens: englishDriversTokens,
    });
  }
});

// NEW OFFER
customEventEmitter.on('newOffer', async (data: NewOfferType) => {
  // 1- SEND NOTIFICATION TO USER ON SOCKET
  let Vehicles;
  if (
    data.offer.Driver &&
    data.offer.Driver.Vehicles &&
    data.offer.Driver.Vehicles.length > 0
  ) {
    Vehicles = serializeVehicle(data.offer.Driver.Vehicles[0], data.language);
  }
  const { Driver, ...offer } = data.offer;
  io.to(data.roomId).emit('new offer', {
    ...offer,
    Driver: {
      ...Driver,
      Vehicles,
    },
  });
  // 2- SEND PUSH NOTIFICATION TO USER
  if (data.fcm_tokens.length > 0) {
    await sendPushNotification({
      title: data.language === 'ar' ? 'عرض جديد' : 'New Offer',
      body: data.language === 'ar' ? 'لديك عرض جديد' : 'You have a new offer',
      tokens: data.fcm_tokens.map((token) => token.token),
    });
  }
});

// OFFER ACCEPTED
customEventEmitter.on('offerAccepted', async (data: AcceptOfferResponse) => {
  // 1- SEND NOTIFICATION TO DRIVER ON SOCKET
  io.to(data.roomId).emit('offer accepted', {});
  // 2- SEND PUSH NOTIFICATION TO DRIVER
  if (data.tokens.length > 0) {
    await sendPushNotification({
      title: data.language === 'ar' ? 'تم قبول عرضك' : 'Offer Accepted',
      body:
        data.language === 'ar'
          ? 'لقد تم قبول عرضك من قبل العميل'
          : 'Your offer has been accepted by the client',
      tokens: data.tokens.map((token) => token.token),
    });
  }
});

// DRIVER CANCELATION
customEventEmitter.on(
  'driverCancelation',
  async (data: DriverCancelationReturn) => {
    const englishDriversTokens: string[] = [];
    const arabicDriversTokens: string[] = [];
    const sockets: string[] = [];
    let arabicBody = '';
    let englishBody = '';
    if (data.type === TripType.VIPTRIP) {
      arabicBody = 'لقد قام السائق بإلغاء الرحله نعمل على توفير سائق بديل';
      englishBody =
        'The driver has cancelled the trip. We are working on providing an alternative driver';
    } else {
      arabicBody = 'تم إلغاء الرحلة من قبل السائق';
      englishBody = 'The trip has been cancelled by the driver';
    }
    data.users.forEach((user) => {
      if (user.prefered_language === 'ar') {
        user.User_FCM_Tokens.forEach((token) => {
          arabicDriversTokens.push(token.token);
        });
      } else {
        user.User_FCM_Tokens.forEach((token) => {
          englishDriversTokens.push(token.token);
        });
      }
      sockets.push(user.uuid);
    });
    // 1- SEND NOTIFICATION TO DRIVER ON SOCKET
    io.to(sockets).emit('driver cancelation', {});
    // 2- SEND PUSH NOTIFICATION TO DRIVER
    if (arabicDriversTokens.length > 0) {
      await sendPushNotification({
        title: 'إلغاء الرحلة',
        body: arabicBody,
        tokens: arabicDriversTokens,
      });
    }
    if (englishDriversTokens.length > 0) {
      await sendPushNotification({
        title: 'Trip Cancelation',
        body: englishBody,
        tokens: englishDriversTokens,
      });
    }
  },
);

// USER CANCELATION
customEventEmitter.on(
  'userCancelation',
  async (data: UserCancelationReturn) => {
    // 1- SEND NOTIFICATION TO USER ON SOCKET
    io.to(data.user.uuid).emit('user cancelation', {});
    // 2- SEND PUSH NOTIFICATION TO USER
    if (
      data.user.User_FCM_Tokens.length > 0 &&
      data.type === TripType.VIPTRIP
    ) {
      await sendPushNotification({
        title:
          data.user.prefered_language === 'ar'
            ? 'إلغاء الرحلة'
            : 'Trip Cancelation',
        body:
          data.user.prefered_language === 'ar'
            ? 'تم إلغاء الرحلة من قبل العميل'
            : 'The trip has been cancelled by the client',
        tokens: data.user.User_FCM_Tokens.map((token) => token.token),
      });
    } else if (
      data.user.User_FCM_Tokens.length > 0 &&
      data.type === TripType.BASICTRIP
    ) {
      await sendPushNotification({
        title:
          data.user.prefered_language === 'ar'
            ? 'إلغاء الحجز'
            : 'Booking Cancelation',
        body:
          data.user.prefered_language === 'ar'
            ? 'تم إلغاء المقعد في الرحلة'
            : 'The seat in the trip has been cancelled',
        tokens: data.user.User_FCM_Tokens.map((token) => token.token),
      });
    }
  },
);

// DRIVER ON THE WAY
// DRIVER ARRIVED
// TRIP STARTED
// TRIP COMPLETED
customEventEmitter.on(
  'tripStatusUpdate',
  async (data: TripStatusChangeReturn) => {
    const englishDriversTokens: string[] = [];
    const arabicDriversTokens: string[] = [];
    const sockets: string[] = [];
    const messages = tripStatusMessages(data.status);
    data.users.forEach((user) => {
      if (user.prefered_language === 'ar') {
        user.User_FCM_Tokens.forEach((token) => {
          arabicDriversTokens.push(token.token);
        });
      } else {
        user.User_FCM_Tokens.forEach((token) => {
          englishDriversTokens.push(token.token);
        });
      }
      sockets.push(user.uuid);
    });
    // 1- SEND NOTIFICATION TO DRIVER ON SOCKET
    io.to(sockets).emit('trip status change', {
      tripId: data.id,
      status: data.status,
      driverId: data.driverId,
    });
    // 2- SEND PUSH NOTIFICATION TO DRIVER
    if (arabicDriversTokens.length > 0) {
      await sendPushNotification({
        title: messages.arabictitle,
        body: messages.arabicBody,
        tokens: arabicDriversTokens,
      });
    }
    if (englishDriversTokens.length > 0) {
      await sendPushNotification({
        title: messages.englishtitle,
        body: messages.englishBody,
        tokens: englishDriversTokens,
      });
    }
  },
);

customEventEmitter.on('newWarning', async (data: Warning) => {
  const { location, ...warning } = data;

  const users: any = await prisma.$queryRaw`
    SELECT *
    FROM (
      SELECT u.uuid, u.prefered_language, ARRAY_AGG(uft.token) AS tokens,
        2 * 6371 * asin(
          sqrt(
            (sin(radians((CAST(location->>'lat' AS FLOAT) - ${+location.lat}) / 2))) ^ 2
            + cos(radians(${+location.lat})) * cos(radians(CAST(location->>'lat' AS FLOAT)))
            * (sin(radians((CAST(location->>'lng' AS FLOAT) - ${+location.lng}) / 2))) ^ 2
          )
        ) as distance
      FROM "User" u
      LEFT JOIN "User_FCM_Token" uft ON u.id = uft."userId"
      WHERE u."role" = ${Roles.DRIVER}
      GROUP BY u.uuid, u.prefered_language, u.location
    ) AS subquery 
    WHERE distance < 100
    ORDER BY distance
  `;
  const arabicFCMToken: string[] = [];
  const englishFCMToken: string[] = [];

  users.forEach((user: any) => {
    if (user.prefered_language === 'ar') {
      arabicFCMToken.push(...user.tokens);
      io.to(user.uuid).emit('newWarning', {
        type: warning.ar_type,
        distance: user.distance,
        location: data.location,
      });
    } else {
      englishFCMToken.push(...user.tokens);
      io.to(user.uuid).emit('newWarning', {
        type: warning.en_type,
        distance: user.distance,
        location: data.location,
      });
    }
  });

  if (englishFCMToken.length > 0) {
    sendPushNotification({
      title: 'New Warning',
      body: warning.en_type,
      tokens: englishFCMToken,
    });
  }
  if (arabicFCMToken.length > 0) {
    sendPushNotification({
      title: 'تحذير جديد',
      body: warning.ar_type,
      tokens: arabicFCMToken,
    });
  }
});

customEventEmitter.on('endTrip', async (data: CreateWaslTrip) => {
  await waslService.createTripRegistration(data);
});

customEventEmitter.on('user-signup', async (user: User) => {
  await authService.sendVerificationCode(user.id);
});
